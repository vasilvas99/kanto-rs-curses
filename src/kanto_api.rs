...
use containers::github::com::eclipse_kanto::container_management::containerm::api::services::containers::{self as cm_rpc, CreateContainerRequest, CreateContainerResponse};
...
pub async fn create_container(channel: &mut ClientChannel, name: &str, registry: &str) -> Result<CreateContainerResponse> {
    ...
    let _r = tonic::Request::new(cm_rpc::CreateContainerRequest{container: Some(template)});
...
pub async fn get_container_by_name(channel: &mut ClientChannel, name: &str) -> Result<Container> {
    ...
    let cont = all_containers.into_iter().find(|c| {
        c.name == name
